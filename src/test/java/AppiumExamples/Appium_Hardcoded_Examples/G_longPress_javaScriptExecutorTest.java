...
public class G_longPress_javaScriptExecutorTest extends C_BaseTest_Emulator_ApiDemosAppTest {
	...
	public void longPressGesture() throws MalformedURLException, URISyntaxException
	{
		...
		//5- Ensure, "Sample menu" is displayed in the drop down.
		/*
		...
		Assert.assertTrue(arrayList.contains(expectedText), "EXCEPTION: Drop down does NOT contain the text of: '" + expectedText + "' . It instead contains "
				+ "the texts of: " +arrayList);
		//without the custom message, the exception will be:    ""expected [true] but found [false]""
		
		*/
		
		/* Or use: 
		Assert.assertListContainsObject(arrayList, expectedText, "Add the exact same msg in the above assertion");
		*/
		
		//6- Ensure that "Sample action" is present in the drop down list.
		String expectedItems[]= {"SAMple actIon" , "sample menU" }; // Convert later only as soon as needed to an arrayList (Even if inside a for-loop). This is due to memory management
		                                           // Even if there is only 1 item, never declare a string (For maintenance if future item(s) is/ are added).
		
		// Now, the logic is to get the text of ALL box elements present in the drop down, and storing them into an arrayList so that
		...
		// Now, in order to use the .contains()/.containsAll() methods, the array (fixed-length data structure) declared at the very beginning
		 // must be converted into an arrayList.==>
		...
		// Plus in order to negate case sensitivity and any added space(s) before/after each string do the following: 
		 ...
		 Assert.assertTrue(arrayList.containsAll(finalexpectedItems_ArrayList), "The displayed list of: " + arrayList + " does NOT contain: "+ finalexpectedItems_ArrayList);
		...
		//8- Ensure that the UI drop down, only contains "Sample menu" & "Sample action" IN ORDER.
		 ...
		 Assert.assertEquals(arrayList, finalexpectedItems_ArrayList, "The actual displayed list of: " + arrayList + " does NOT equal/[ NOT in order] the expected on of: " + finalexpectedItems_ArrayList );
		...
		//9- Ensure that the UI drop down, only contains "Sample menu" & "Sample action" IN NO PARTICULAR ORDER.
		 ...
		 Assert.assertEqualsNoOrder(arrayList, finalexpectedItems_ArrayList, "The actual displayed list of: " + arrayList + " does NOT equal the expected on of: " + finalexpectedItems_ArrayList);
